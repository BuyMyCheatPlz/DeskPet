...
开启「宠物鼠标穿透」后，鼠标（光标）一旦靠近或落到模型区域，宠物会**立刻、高速朝远离鼠标的方向躲开**，避免挡住鼠标（例如打游戏时鼠标快速移动、或要点击模型背后的窗口）。光标离开模型区域即停下。

- 检测用 Win32 `GetCursorPos`（物理像素）换算为 DIP 坐标——比 WPF 的 `Mouse.GetPosition` 更可靠（在透明穿透窗口上后者可能恒返回 `(0,0)`）。
- 检测频率约 **16ms**，命中区在模型四周**外扩约 50px**，提前触发。
- 躲避速度约 **450px/s**（正常步行速度约 70px/s 的 6 倍多），一次躲开约 2 个模型宽度的距离。
- 仅穿透开启时生效；关闭穿透后模型恢复正常，可正常点击、拖动。
...
- **清洁度（Cleanliness）**：卫生值，随时间衰减；**回巢（走到悬浮窗并淡出）后以下降速度的 5 倍逐步恢复**。暂不参与行为判断。

### 随时间变化（每 60 秒结算一次）

- **清洁度**：每 60 秒下降 `0.8 × 属性衰减速度`（默认速度 0.5 → 每次 -0.4），最低 0；**回巢后每 60 秒上升 `4 × 属性衰减速度`（即下降速度的 5 倍，默认 → 每次 +2），直到 100**。
