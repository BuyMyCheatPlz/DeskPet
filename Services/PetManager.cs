    ...
    private void AnimTick()
    {
        ...
        switch (_movementMode)
        {
            case PetMovementMode.Floor:
                // Don't move until playback reaches the configured move-start index
                // (decoupled from the loop start — the pet may start walking while
                // still in the startup frames).
                int moveStartIdx = Action == PetAction.Walk
                    ? (Skin?.MoveStart(PetAction.Walk, _currentVariant) ?? 0)
                    : 0;
                bool moveNotAllowed = moveStartIdx > 0 && _frameProgress < moveStartIdx;
                if (_walkTarget is { } target && !_isDragging && !_wasMusicPlaying && !moveNotAllowed
        ...
        if (isLoop)
        {
            // Loop from the configured loop-start index onward; frames before it
            // play once as startup (so a walk lead-in does not repeat).
            int loopStart = Skin?.LoopStart(Action, _currentVariant) ?? 0;
            if (loopStart > 0 && loopStart < count)
            {
                int loopLen = count - loopStart;
                int idx = _frameProgress < loopStart
                    ? (int)_frameProgress
                    : loopStart + (int)((_frameProgress - loopStart) % loopLen);
                if (Math.Min(idx, count - 1) != _lastFrameIndex)
                {
                    CurrentFrame = frames[Math.Min(idx, count - 1)];
                    _lastFrameIndex = Math.Min(idx, count - 1);
                }
            }
            else
            {
